...
// import { QuizService } from "@/services/quiz.service"; // لما تحب تربط الـ API
...
export default function JoinQuizModal({ isOpen, onClose }: JoinQuizModalProps) {
  ...
  const handleJoin = async () => {
    ...
    setLoading(true);
    try {
      // هنا هتربط بالـ API للتحقق من الكود
      // const res = await QuizService.joinQuiz(quizCode);
      toast.success(`Joining quiz with code: ${quizCode}`);
      onClose();
    } catch (error) {
