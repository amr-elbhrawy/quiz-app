...
export function Home({ setActive }: HomeProps) {
  const dispatch = useDispatch<AppDispatch>();
  const { incoming: quizzes, error, successMessage } = useSelector((state: RootState) => state.quiz);
  ...
  const actionButtons = useMemo(() => (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <ActionButton icon={FaRegClock} title="Set up a new quiz" onClick={handleOpenModal} />
      <ActionButton icon={GiSave} title="Question Bank" onClick={handleNavigateToQuestions} />
      <ActionButton icon={FaUsers} title="All Quizzes" onClick={handleNavigateToQuizzes} />
    </div>
  ), [handleOpenModal, handleNavigateToQuestions, handleNavigateToQuizzes]);

  if (selectedQuizId) {
    return (
      <QuizDetails quizId={selectedQuizId} onBack={handleBackToList} />
    );
  }
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-6">
          <UpcomingQuizzes onOpenQuiz={handleOpenQuiz} onJoin={handleOpenQuiz} refreshTrigger={refreshTrigger} />
          <CompletedQuizzes />
        </div>
      </div>
